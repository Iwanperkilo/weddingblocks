...
function weddingblocks_register_blocks()
{
    ...
    /**
     * Extension point: an add-on plugin (e.g. WeddingBlocks Pro) can
     * register its own Gutenberg blocks here, right after Free's own
     * blocks are registered. Free knows nothing about what gets hooked
     * in here — it's just a place in the timing to do it safely.
     */
    do_action('weddingblocks_register_additional_blocks');
}
...
function weddingblocks_render_core_block_animations($block_content, $block)
{
    ...
    $allowed_blocks = apply_filters('weddingblocks_animatable_core_blocks', array(
        'core/paragraph',
        'core/heading',
        'core/image',
        'core/group',
    ...
    // Entrance (scroll-triggered, one-shot) and attention (continuous/looping)
    // animations are independent attribute namespaces, so a block can use
    // either one on its own or both at the same time.
    $anim_attrs = weddingblocks_get_animation_attrs($attrs);
    $attn_attrs = weddingblocks_get_attention_attrs($attrs);

    if (empty($anim_attrs) && empty($attn_attrs)) {
        return $block_content;
    }

    $combined_attrs = array_merge($anim_attrs, $attn_attrs);

    // Build the data attributes string
    $data_str = '';
    foreach ($combined_attrs as $key => $val) {
